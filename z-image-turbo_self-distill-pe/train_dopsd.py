...
def main(args):
    ...
    # the SAME text encoder as the student (which uses the short prompt p0). No VLM / no
    # target-image context is used here -- this is the only conceptual change vs D-OPSD.
            ...
            with accelerator.accumulate(gen_model):
                ...
                with torch.no_grad():
                    with accelerator.autocast():
                        ...
                        # <-- THE ONLY CONCEPTUAL CHANGE vs D-OPSD: replace the multimodal
                        #     (prompt + target image) teacher context with the enhanced prompt.
                        prompt_embeds_list_t = _encode_prompt(
                            pipeline.text_encoder,
                            tokenizer,
                            teacher_prompts,
                            max_sequence_length=512,
                ...
                for back_step in range(len(timesteps)):
                    ...
                    # teacher (conditioned on the enhanced prompt p1)
                    with torch.no_grad():
                        with accelerator.autocast():
