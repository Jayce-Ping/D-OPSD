...
from utils import create_validation_generators
...
def main(args):
    ...
    # Prompt-enhance teacher: the teacher condition is the ENHANCED prompt p1 encoded by
    # the SAME text encoder as the student (which uses the short prompt p0). No edit branch /
    # no target-image context is used here -- this is the only conceptual change vs the
    # edit-context D-OPSD variant.
    ...
    # (student_key, teacher_key) pairs: student -> p0 (short prompt), teacher -> p1 (enhanced prompt)
    prompt_key_pairs = parse_prompt_key_pairs(args.prompt_key_pairs)
    if accelerator.is_main_process:
        logger.info(f"Prompt-enhance pairs (p0 -> p1): {prompt_key_pairs}")
    ...
    # validation dataset: returns (student_prompt p0, teacher_prompt p1) pairs
    num_test_samples = args.batch_size_test * accelerator.num_processes
    test_dataset = TextPromptDataset(
        str(test_jsonl_path),
        student_prompt_key=args.student_prompt_key,
        teacher_prompt_key=args.teacher_prompt_key,
    ...
    # get sample prompts (p0 for the student, p1 for the teacher)
    test_student_prompts, test_teacher_prompts = next(iter(test_dataloader))
    ...
    for epoch in range(epoch_start + 1, args.epochs):
        for batch in train_dataloader:
            ...
            with accelerator.accumulate(gen_model):
                ...
                with torch.no_grad():
                    with accelerator.autocast():
                        # student condition  c_s = f(p0)  (short prompt)
                        ...
                        # teacher condition  c_t = f(p1)  (enhanced prompt)
                        # <-- THE ONLY CONCEPTUAL CHANGE vs the edit-context variant:
                        #     replace the edit-branch (prompt + reference image) teacher
                        #     context with the enhanced prompt through the same encoder.
                ...
                for back_step in range(len(timesteps)):
                    ...
                    # teacher (conditioned on the enhanced prompt p1, no image latents)
                    with torch.no_grad():
                        with accelerator.autocast():
